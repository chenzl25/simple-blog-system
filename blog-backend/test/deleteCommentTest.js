...
describe('add post', function() {
  ...
  var commentId;
	...
	it('should edit comment faid for the wrong commentId', function(done) {
	 agent2.delete('/api/post/'+postId+'/comment/'+commentId+1)
				.then(
					(res) => {
						console.log(res.body);
	...
	it('should edit comment faid for the wrong postId', function(done) {
	 agent2.delete('/api/post/'+postId+1+'/comment/'+commentId+1)
				.then(
					(res) => {
						console.log(res.body);
						expect(res.body.error).equal(true);
						console.log(res.body.message.message);
						expect(res.body.message).equal('删除评论失败'); // 因为ID为ObjectId所以这会先出错先于‘blog不存在’
	...
	it('should edit comment faid for not the commentOwner', function(done) {
	 agent1.delete('/api/post/'+postId+'/comment/'+commentId)
				.then(
					(res) => {
						console.log(res.body);
						expect(res.body.error).equal(true);
	...
	it('should delete comment successfully', function(done) {
	 agent2.delete('/api/post/'+postId+'/comment/'+commentId)
				.then(
					(res) => {
						console.log(res.body);
						expect(res.body.error).equal(false);
						console.log(res.body.message);
						expect(res.body.message).equal('成功删除评论');
						done();
					}
				).catch(function(err) {
					done(err);
